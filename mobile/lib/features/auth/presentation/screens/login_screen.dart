...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  String _friendlyGoogleError(Object? err) {
    ...
    if (err is DioException && err.response?.statusCode == 409) {
      return 'Ya existe una cuenta con este correo. Inicia sesión con tu correo y contraseña.';
    }
    final msg = _googleErrorMessage(err);
    if (msg.contains('invalid_client') || msg.contains('no registered origin')) {
      return 'Google no autoriza este origen. Configura el cliente OAuth Web.';
    }
    if (msg.contains('client id is missing') || msg.contains('oauth web client')) {
      return 'Falta configurar el cliente OAuth Web de Google.';
    }
